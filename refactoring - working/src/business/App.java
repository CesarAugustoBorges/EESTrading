...
public class App {
    ...
    public int unitsRemaining(int id_stock) {
        ...
        for (Position pst : positionDAO.values()) {
            if (isPositionToBuyAndDealt(id_stock, pst))
                units = pst.getUnits_remaining();
        }
        return units;
    }

    private boolean isPositionToBuyAndDealt(int id_stock, Position pst) {
    ...
    private Position openBuyPosition(String stockname, int amount, float stop_loss, float take_profit, int sz) {
        float dv;
